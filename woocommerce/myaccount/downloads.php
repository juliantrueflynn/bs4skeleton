							...
							switch ( $column_id ) :
								...
								case 'download-expires' :
									if ( ! empty( $download['access_expires'] ) ) :
										?>
										<time datetime="<?php echo date( 'Y-m-d', strtotime( $download['access_expires'] ) ); // WPCS: XSS OK. ?>" title="<?php echo esc_attr( strtotime( $download['access_expires'] ) ); ?>">
											<?php echo date_i18n( get_option( 'date_format' ), strtotime( $download['access_expires'] ) ); // WPCS: XSS OK. ?>
										</time>
										<?php
									else :
								...
								case 'download-actions' :
									...
									if ( $actions ) :
										foreach ( $actions as $key => $action ) :
											?>
											<a href="<?php echo esc_url( $action['url'] ); ?>" <?php bs4_btn_class( 'button woocommerce-Button ' . esc_attr( $key ) ); ?>><?php echo esc_html( $action['name'] ); ?></a>
											<?php
										endforeach;
									endif;
									break;
