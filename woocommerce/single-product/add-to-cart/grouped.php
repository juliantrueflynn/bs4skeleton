...
<form class="cart" method="post" enctype='multipart/form-data'>
		...
		<tbody>
			...
			foreach ( $grouped_products as $grouped_product ) {
				...
				<tr id="product-<?php the_ID(); ?>" <?php post_class(); ?>>
					...
					<td class="label">
						<label for="product-<?php echo $grouped_product->get_id(); // WPCS: XSS OK. ?>">
							<?php echo $grouped_product->is_visible() ? '<a href="' . esc_url( apply_filters( 'woocommerce_grouped_product_list_link', get_permalink( $grouped_product->get_id() ), $grouped_product->get_id() ) ) . '">' . $grouped_product->get_name() . '</a>' : $grouped_product->get_name(); // WPCS: XSS OK. ?>
						</label>
					</td>
					<?php do_action( 'woocommerce_grouped_product_list_before_price', $grouped_product ); ?>
					<td class="price">
						<?php
							echo $grouped_product->get_price_html(); // WPCS: XSS OK.
							echo wc_get_stock_html( $grouped_product ); // WPCS: XSS OK.
						?>
					</td>
				</tr>
				<?php
	...
	<?php if ( $quantites_required ) : ?>
		...
		<button type="submit" <?php bs4_btn_class(); ?>><?php echo esc_html( $product->single_add_to_cart_text() ); ?></button>
