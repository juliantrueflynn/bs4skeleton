...
<div id="reviews" class="woocommerce-Reviews">
	<div id="comments">
		...
		if ( get_option( 'woocommerce_enable_review_rating' ) === 'yes' && ( $count ) ) :
			...
			printf( esc_html( _n( '%1$s review for %2$s', '%1$s reviews for %2$s', $count, 'bs4' ) ), esc_html( $count ), '<span>' . get_the_title() . '</span>' );
		else :
			esc_attr_e( 'Reviews', 'bs4' );
		endif;
		?></h2>
		...
		<?php else : ?>
			...
			<p class="woocommerce-noreviews"><?php esc_attr_e( 'There are no reviews yet.', 'bs4' ); ?></p>
	...
	<?php if ( get_option( 'woocommerce_review_rating_verification_required' ) === 'no' || wc_customer_bought_product( '', get_current_user_id(), $product->get_id() ) ) : ?>
		...
		<div id="review_form_wrapper">
			<div id="review_form">
				...
				$comment_form = array(
					'title_reply' => have_comments() ? __( 'Add a review', 'bs4' ) : sprintf(
						/* translators: %s product title. */
						esc_attr__( 'Be the first to review &ldquo;%s&rdquo;', 'bs4' ),
						get_the_title()
					),
				...
				if ( $account_page_url ) :
					$comment_form['must_log_in'] = '<p class="must-log-in">' . sprintf(
						/* translators: %s product title. */
						__( 'You must be <a href="%s">logged in</a> to post a review.', 'bs4' ), esc_url( $account_page_url )
					) . '</p>';
				endif;

				if ( get_option( 'woocommerce_enable_review_rating' ) === 'yes' ) :
					$ratings_options = array(
						'' => esc_html__( 'Rate&hellip;', 'bs4' ),
					...
					$comment_form['comment_field'] .= bs4_form_label( esc_html__( 'Your rating', 'bs4' ), 'rating' );
	...
	<?php else : ?>
		...
		<p class="woocommerce-verification-required"><?php esc_attr_e( 'Only logged in customers who have purchased this product may leave a review.', 'bs4' ); ?></p>
